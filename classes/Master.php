...
Class Master extends DBConnection {
	...
	function save_category(){
		...
		$check = $this->conn->query("SELECT * FROM `category_list` where `name` = '{$name}' and seller_id = '{$seller_id}' and delete_flag = 0 ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
	...
	function save_product(){
		...
		$check = $this->conn->query("SELECT * FROM `product_list` where seller_id = '{$seller_id}' and `name` = '{$name}' and delete_flag = 0 ".(!empty($id) ? " and id != '{$id}'" : ""))->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
	...
	function place_order(){
		...
		$has_failed=false;
		$gtotal = 0;
		$vendors = $this->conn->query("SELECT * FROM `seller_list` where id in (SELECT seller_id from product_list where id in (SELECT product_id FROM `cart_list` where client_id ='{$this->settings->userdata('id')}')) order by `shop_name` asc");
		$prefix = date('Ym-');
		$code = sprintf("%'.05d",1);
		while($vrow = $vendors->fetch_assoc()):
			...
			$sql = "INSERT INTO `order_list` (`code`,`client_id`,`seller_id`,`delivery_address`) VALUES {$data}";
			$save = $this->conn->query($sql);
			if($save){
				...
				$data = "";
				$gtotal = 0 ;
				$products = $this->conn->query("SELECT c.*, p.name as `name`, p.price,p.image_path FROM `cart_list` c inner join product_list p on c.product_id = p.id where c.client_id = '{$this->settings->userdata('id')}' and p.seller_id = '{$vrow['id']}' order by p.name asc");
