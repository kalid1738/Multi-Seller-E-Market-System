    ...
    <div class="card card-outline card-primary rounded-0 shadow-0">
        ...
        <div class="card-body">
            <div id="cart-list">
                ...
                <?php 
                $gtotal = 0;
                $vendors = $conn->query("SELECT * FROM `seller_list` where id in (SELECT seller_id from product_list where id in (SELECT product_id FROM `cart_list` where client_id ='{$_settings->userdata('id')}')) order by `shop_name` asc");
                while($vrow=$vendors->fetch_assoc()):                
                ?>
                    <div class="col-12 border">
                        <span>Seller: <b><?= $vrow['code']. " - " . $vrow['shop_name'] ?></b></span>
                    </div>
                    <div class="col-12 border p-0">
                        <?php 
                        $vtotal = 0;
                        $products = $conn->query("SELECT c.*, p.name as `name`, p.price,p.image_path FROM `cart_list` c inner join product_list p on c.product_id = p.id where c.client_id = '{$_settings->userdata('id')}' and p.seller_id = '{$vrow['id']}' order by p.name asc");
               ...
               <!--  <div class="col-12 border">
                        <div class="d-flex">
                             <div class="col-9 text-right font-weight-bold text-muted">VAT</div>
