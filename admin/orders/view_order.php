...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT o.*,c.code as ccode, CONCAT(c.lastname, ', ',c.firstname,' ') as client, CONCAT(v.code, '-',v.shop_name) as `vendor` from `order_list` o inner join client_list c on o.client_id = c.id inner join seller_list v on o.seller_id= v.id where o.id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
