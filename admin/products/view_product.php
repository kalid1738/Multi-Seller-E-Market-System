...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT p.*, c.name as `category`,v.code, v.shop_name as `vendor` from `product_list` p inner join category_list c on p.category_id = c.id inner join seller_list v on p.seller_id = v.id where p.id = '{$_GET['id']}' and p.delete_flag = 0 ");
    if($qry->num_rows > 0){
...
<div class="container-fluid">
	<center><img src="<?= validate_image(isset($image_path) ? $image_path : "") ?>" alt="Product" class="img-thubmnail p-0 bg-gradient-gray" id="prod-img-view"></center>
	<dl>
